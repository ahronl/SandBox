...
namespace TimerWrapper.Log
{
    internal class ZTimerLogger : ZTimerLoggerBase
    {
        ...
        {
            s_logger.Error(ex, $"ZTimer encounter an error Timer Id : {sender.Id} Timer name: {sender.Name} Timer GUID: {sender.InstanceId}");
        }

        protected override void LogCanceledEvent(ITimer sender)
        {
            s_logger.Info($"ZTimer canceled execution Timer Id :{ sender.Id}, Timer name : {sender.Name} Timer GUID: {sender.InstanceId}");
        }

        protected override void LogStopEvent(ITimer sender)
        {
            s_logger.Info($"ZTimer stopped Timer Id :{sender.Id}, Timer name : {sender.Name} Timer GUID: {sender.InstanceId}");
        }

        protected override void LogStartEvent(ITimer sender)
        {
            s_logger.Info($"ZTimer started Timer Id :{sender.Id}, Timer name : {sender.Name} Timer GUID: {sender.InstanceId}");
        }
    }
}
